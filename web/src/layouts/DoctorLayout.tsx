...
export default function DoctorLayout() {
  ...
  return (
    <div className="w-full h-screen flex bg-background text-textPrimary font-sans overflow-hidden">
      {/* LEFT SIDEBAR (Desktop Only) */}
      <aside className={`bg-background border-border flex flex-col flex-shrink-0 relative z-20 transition-all duration-300 hidden md:flex ${isSidebarExpanded ? 'w-64 border-r' : 'w-0 opacity-0 overflow-hidden border-r-0'}`}>
        <div className={`h-16 flex items-center px-4 justify-between border-b border-border w-64`}>
          <div className="flex items-center overflow-hidden">
            <div className="w-8 h-8 shrink-0 rounded-lg bg-gradient-to-br from-[#00C9A7] to-[#009E83] flex items-center justify-center font-bold text-textPrimary shadow-lg shadow-[#00C9A7]/20 mr-3">
              <Stethoscope size={18} />
            </div>
            <h1 className="text-xl font-bold tracking-tight text-textPrimary whitespace-nowrap">LifeLink<span className="text-emerald-400">AI</span></h1>
          </div>
          {isSidebarExpanded && (
            <button
        ...
        {isSidebarExpanded && (
           <div className="p-4 border-t border-border bg-gradient-to-br from-[#0f172a] to-[#020617]">
              <div className="bg-gradient-to-r from-purple-600/20 to-blue-600/20 border border-purple-500/30 p-4 rounded-xl relative overflow-hidden">
                 ...
                 <h4 className="text-textPrimary font-bold text-sm mb-1 relative z-10">Upgrade Your Practice</h4>
                 <p className="text-textSecondary text-xs mb-3 relative z-10">Unlock more patients and advanced features.</p>
                 <button className="w-full py-2 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg text-xs font-bold text-textPrimary relative z-10 hover:shadow-lg hover:shadow-purple-500/30 transition-all">Upgrade Now</button>
              </div>
           </div>
        )}
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-background relative">
        {/* TOP HEADER */}
        <header className="h-16 bg-background border-b border-border flex items-center justify-between px-4 md:px-6 flex-shrink-0 z-10">
          <div className="flex items-center gap-2 md:gap-4">
            {/* Mobile Hamburger */}
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="md:hidden w-10 h-10 rounded-lg hover:bg-surface flex items-center justify-center text-textSecondary transition-colors"
            >
              <Menu size={24} />
            </button>
            ...
            <div className="flex items-center md:hidden gap-2">
              <div className="w-7 h-7 rounded bg-gradient-to-br from-[#00C9A7] to-[#009E83] flex items-center justify-center text-textPrimary shadow-lg shadow-[#00C9A7]/20">
                <Stethoscope size={14} />
              </div>
              <h1 className="text-lg font-bold tracking-tight text-textPrimary whitespace-nowrap">LifeLink</h1>
            </div>
            ...
            {!isSidebarExpanded && (
              <button
                ...
                className="hidden md:flex w-8 h-8 rounded-lg hover:bg-surface items-center justify-center text-textSecondary transition-colors"
              >
                <Menu size={20} />
              </button>
            )}
            <h2 className="text-xl font-bold text-textPrimary hidden md:block ml-2">Doctor Portal</h2>
          </div>
          ...
          <div className="flex items-center gap-4 pl-4">
            ...
            <div className="flex items-center gap-3 cursor-pointer">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-textPrimary leading-tight">Dr. {user?.fullName || 'Demo User'}</p>
                <p className="text-xs text-textSecondary">Internal Medicine</p>
              </div>
            ...
            <button
              ...
              className={`hidden sm:flex px-5 py-2 rounded-lg border transition-all font-bold text-sm items-center gap-2 ${
                isOnline
                  ? 'bg-rose-500/10 text-rose-500 border-rose-500/30 hover:bg-rose-500 hover:text-textPrimary'
                  : 'bg-emerald-600 text-textPrimary border-transparent hover:bg-emerald-500 shadow-lg shadow-emerald-600/20'
              }`}
            >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute inset-0 z-40 bg-black/50 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)}>
            <div 
              className="absolute left-0 top-0 bottom-0 w-64 bg-background border-r border-border shadow-2xl flex flex-col"
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-lg border transition-all font-bold text-sm flex justify-center items-center gap-2 ${
                    isOnline
                      ...
                      : 'bg-emerald-600 text-textPrimary border-transparent shadow-lg shadow-emerald-600/20'
                  }`}
                >
                  {isOnline ? <><VideoOff size={16} /> Go Offline</> : <><Video size={16} /> Go Online</>}
                </button>
        ...
        <nav className="md:hidden absolute bottom-0 left-0 right-0 h-16 bg-background backdrop-blur-md border-t border-border z-30 flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom)]">
          {[
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex flex-col items-center justify-center w-16 h-full gap-1 transition-colors relative ${
                  isActive ? 'text-emerald-400' : 'text-textTertiary hover:text-textSecondary'
                }`
              }
            >
              {({ isActive }) => (
