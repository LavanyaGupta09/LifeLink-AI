...
const PlanCard: React.FC<PlanCardProps> = ({ plan, matchPercentage }) => {
  ...
  return (
    <div className="bg-card rounded-3xl p-5 border border-border hover:border-[#3D91FF]/50 transition-all flex flex-col relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start mb-4 pr-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-textPrimary leading-tight">{plan.planName}</h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <p className="text-[10px] text-textTertiary uppercase">Coverage</p>
          <p className="font-bold text-lg text-textPrimary">₹{(plan.coverage / 100000).toString()} Lakh</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-textTertiary uppercase">Premium</p>
          <p className="font-bold text-lg text-textPrimary">₹{plan.monthlyPremium}<span className="text-xs text-textSecondary">/mo</span></p>
        </div>
      </div>
          ...
          <button 
            ...
            className={`flex-1 border py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1 transition-colors ${
              isCompared 
                ...
                : 'bg-background border-border text-textSecondary hover:bg-surface'
            }`}
          >
        ...
        <button 
          ...
          className={`w-full py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-colors ${
            isSaved 
              ...
              : 'bg-transparent text-textSecondary hover:text-textPrimary border border-transparent hover:border-border'
          }`}
        >
