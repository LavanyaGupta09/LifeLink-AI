...
export default function LifeLinkAIAssistant() {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed inset-0 md:inset-auto md:bottom-28 md:right-8 md:w-[400px] md:h-[600px] z-[100] bg-background md:bg-background md:border border-border md:rounded-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-full md:slide-in-from-bottom-12 duration-300">
          ...
          <div className="bg-card border-b border-border p-4 pt-[env(safe-area-inset-top,16px)] flex items-center justify-between rounded-t-3xl">
            ...
            <button 
              ...
              className="w-8 h-8 bg-surface rounded-full flex items-center justify-center text-textSecondary hover:text-textPrimary transition-colors"
            >
              <ChevronDown size={18} />
            </button>
          </div>
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold transition-all border text-left
                            ${isPrimary 
                              ? 'bg-red-500 hover:bg-red-600 text-textPrimary border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.3)]' 
                              : 'bg-card hover:bg-surface text-textSecondary border-border'
                            }`}
                        >
          ...
          <div className="p-3 bg-card border-t border-border pb-[max(env(safe-area-inset-bottom,12px),12px)] md:rounded-b-3xl">
            <div className="relative flex items-end bg-background border border-border rounded-2xl p-1 focus-within:border-[#3D91FF]/50 transition-colors">
              <textarea
                ...
                className="w-full bg-transparent border-none text-textPrimary text-sm px-3 py-2.5 resize-none max-h-32 min-h-[44px] focus:outline-none scrollbar-hide"
                rows={1}
              />
              <button
                onClick={handleSend}
