...
export default function PhysioOnline() {
  ...
  if (activeCall) {
    return (
      <div className="fixed inset-0 bg-background z-50 flex flex-col justify-between overflow-hidden">
        ...
        <div className="relative z-10 p-6 flex justify-between items-start pt-[env(safe-area-inset-top,24px)]">
          <div>
            <div className="bg-black/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 w-fit mb-4">
              ...
              <span className="text-textPrimary text-sm font-bold tracking-widest">04:12</span>
            </div>
            <h2 className="text-3xl font-bold text-textPrimary drop-shadow-md">{activeCall.name}</h2>
        ...
        <div className="relative z-10 p-6 flex flex-col gap-6 pb-[env(safe-area-inset-bottom,24px)]">
          <div className="self-end w-32 h-44 bg-surface rounded-2xl border-2 border-emerald-500/50 overflow-hidden shadow-2xl relative">
          ...
          <div className="flex justify-center items-center gap-6 mt-4">
            <button className="w-14 h-14 rounded-full bg-surface backdrop-blur-md border border-white/10 flex items-center justify-center text-textPrimary active:scale-95 transition-transform">
              <MicOff size={24} />
            </button>
            <button className="w-14 h-14 rounded-full bg-surface backdrop-blur-md border border-white/10 flex items-center justify-center text-textPrimary active:scale-95 transition-transform">
              <VideoOff size={24} />
            </button>
            <button onClick={() => setActiveCall(null)} className="w-16 h-16 rounded-full bg-rose-600 flex items-center justify-center text-textPrimary shadow-[0_0_30px_rgba(225,29,72,0.5)] active:scale-95 transition-transform">
              <PhoneOff size={28} />
            </button>
          </div>
        </div>
    ...
    <div className="w-full min-h-screen bg-background text-textPrimary font-sans flex flex-col pb-[120px] px-6 py-6">
      ...
      <div className="flex-1 flex flex-col gap-4">
        {ONLINE_PHYSIOS.map(p => (
          <div key={p.id} className="bg-card border border-border rounded-3xl p-5 shadow-xl relative overflow-hidden group">
            <div className="flex gap-4 mb-4">
              ...
              <div className="flex-1 pt-1">
                <h4 className="font-bold text-textPrimary text-lg leading-tight">{p.name}</h4>
            ...
            <div className="flex items-center justify-between border-t border-border pt-4">
              <div>
                <p className="text-[10px] font-bold text-textTertiary uppercase">Consultation Fee</p>
                <p className="font-black text-textPrimary text-base">₹{p.fee}</p>
              </div>
              <button 
