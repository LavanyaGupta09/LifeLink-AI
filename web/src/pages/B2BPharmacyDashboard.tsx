  ...
  return (
    <div className={`w-full min-h-screen bg-background text-textPrimary font-sans flex flex-col relative pb-[120px] md:pb-0 px-6 py-6 transition-all duration-300 ${isSidebarExpanded ? 'md:pl-72' : ''}`}>
      ...
      <header className="w-full flex justify-between items-center p-6 lg:px-10 lg:py-8 border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-textSecondary text-sm md:text-base mt-1">Central Fulfillment • Delhi NCR</p>
          </div>
        </div>
      ...
      <main className="w-full flex-1 p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="w-full bg-card border border-border rounded-3xl p-6 lg:p-8 shadow-xl lg:col-span-8 flex flex-col">
          <div className="flex justify-between items-center mb-6 border-b border-border pb-4">
            <h3 className="text-textPrimary font-bold flex items-center gap-2 text-xl">📦 Pending e-Prescriptions</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
            {orders.length === 0 ? (
               <div className="col-span-1 md:col-span-2 flex flex-col items-center justify-center text-textTertiary py-12">
                 <p className="font-bold tracking-widest uppercase">No pending prescriptions</p>
               </div>
            ) : (
              orders.map((order, idx) => (
                <div key={order.id || idx} className="bg-background rounded-2xl p-6 border border-emerald-500/30 flex flex-col justify-between shadow-lg relative overflow-hidden group">
                   <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
                   <div>
                     <div className="flex justify-between items-start mb-4">
                       <span className="font-black text-emerald-400 text-lg">RX-{String(order.id).padStart(4, '0')}</span>
                       <span className="text-textSecondary text-sm font-bold bg-surface px-2 py-1 rounded flex items-center gap-1">
                          <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></div>
                          {formatTimeAgo(order.created_at)}
                       </span>
                     </div>
                     <h4 className="font-bold text-textPrimary text-xl mb-1">{order.medicine}</h4>
                     <p className="text-textSecondary text-sm font-bold mb-3">{order.dosage} • {order.duration}</p>
                     <p className="text-textTertiary text-sm font-medium border-t border-border pt-3">Patient ID: {order.patient_id}</p>
                   </div>
                   <button 
        ...
        <div className="w-full lg:col-span-4 flex flex-col gap-6 lg:gap-8">
          ...
          {/* INVENTORY MANAGER */}
          <div className="w-full bg-card border border-border rounded-3xl p-6 lg:p-8 shadow-xl">
            <div className="flex justify-between items-center mb-6 border-b border-border pb-4">
              <h3 className="text-textPrimary font-bold flex items-center gap-2 text-xl">💊 Inventory Alerts</h3>
            ...
            <div className="space-y-4">
              <div className="flex bg-background rounded-2xl p-4 items-center justify-between border border-border">
                 <span className="text-base font-medium text-textPrimary">Paracetamol 500mg</span>
              ...
              <div className="flex bg-background rounded-2xl p-4 items-center justify-between border border-border">
                 <span className="text-base font-medium text-textPrimary">Insulin Glargine</span>
          ...
          <div className="grid grid-cols-2 gap-4 flex-1">
            <div className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center justify-center shadow-xl">
               <span className="text-textSecondary text-xs font-bold uppercase mb-2 tracking-wider text-center">Today's Earnings</span>
               <div className="text-3xl lg:text-4xl font-black text-emerald-400">₹450</div>
            </div>
            <div className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center justify-center shadow-xl">
               <span className="text-textSecondary text-xs font-bold uppercase mb-2 tracking-wider text-center">Pending Payout</span>
      ...
      <div className={`fixed bottom-6 md:bottom-auto md:top-0 md:left-0 md:h-full w-[90%] left-[5%] md:left-0 bg-card backdrop-blur-xl border border-border md:border-y-0 md:border-l-0 md:border-r md:rounded-none rounded-full px-4 py-3 md:px-0 md:py-6 flex md:flex-col justify-between md:justify-start items-center gap-0 md:gap-4 shadow-2xl z-50 overflow-x-auto md:overflow-visible transition-transform duration-300 ${isSidebarExpanded ? 'md:w-64 md:items-start md:px-4 md:translate-x-0' : 'md:w-64 md:-translate-x-full'}`}>
        ...
        <button className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl text-textTertiary hover:text-textSecondary md:hover:bg-surface ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'STOCK' : undefined}>
          <span className="text-2xl md:text-xl shrink-0 flex items-center justify-center">💊</span>
        ...
        <button className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl text-textTertiary hover:text-textSecondary md:hover:bg-surface ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'ALERTS' : undefined}>
        ...
        <button className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl text-textTertiary hover:text-textSecondary md:hover:bg-surface ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'PAYOUTS' : undefined}>
          <span className="text-2xl md:text-xl shrink-0 flex items-center justify-center">💸</span>
