      ...
      <header className="w-full flex justify-between items-center p-6 lg:px-10 lg:py-8 border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-textSecondary text-sm md:text-base mt-1">Verified Facility • Delhi NCR</p>
          </div>
        </div>
        <div className="flex gap-3">
           <button onClick={handleLogout} className="px-5 py-3 bg-rose-500/10 text-rose-500 border border-rose-500/30 rounded-xl hover:bg-rose-500 hover:text-textPrimary font-bold transition">Logout</button>
        </div>
      </header>
      ...
      <main className="w-full flex-1 p-6 lg:p-10 flex flex-col items-center justify-center">
        ...
        {!activeTab && (
          <div className="w-full flex-1 flex flex-col items-center justify-center animate-fade-in opacity-80">
            ...
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-textPrimary mb-4">Max Super Speciality</h2>
            <p className="text-textSecondary text-lg md:text-xl max-w-md text-center">Select a module from the sidebar to manage your hospital's operations.</p>
          </div>
        )}
        ...
        {activeTab && (
          <div className="w-full max-w-4xl flex justify-start mb-6 animate-fade-in">
            <button 
              ...
              className="flex items-center gap-2 text-textSecondary hover:text-textPrimary transition-colors font-bold tracking-wider uppercase text-sm bg-card px-4 py-2 rounded-xl border border-border shadow-lg active:scale-95"
            >
        ...
        {activeTab === 'beds' && (
          <div className="w-full max-w-3xl flex flex-col gap-6 animate-fade-in">
            <h2 className="text-2xl font-bold text-textPrimary mb-2">Live Capacity Tracker</h2>
            
            <div className="flex flex-col gap-5">
              {/* ICU */}
              <div className="bg-card border border-border rounded-3xl p-8 flex flex-col sm:flex-row sm:items-center justify-between shadow-lg gap-4">
                 <div>
                   <span className="text-textSecondary text-sm font-bold uppercase tracking-wider block mb-2">ICU Beds</span>
                   <div className="text-5xl font-black text-textPrimary"><span className={icuBeds > 0 ? "text-emerald-400" : "text-red-500"}>{icuBeds}</span><span className="text-3xl text-textTertiary">/10</span></div>
                 </div>
                 <div className="flex gap-3">
                   <button onClick={() => setIcuBeds(Math.max(0, icuBeds - 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">-</button>
                   <button onClick={() => setIcuBeds(Math.min(10, icuBeds + 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">+</button>
                 </div>
              </div>
              ...
              <div className="bg-card border border-border rounded-3xl p-8 flex flex-col sm:flex-row sm:items-center justify-between shadow-lg gap-4">
                 <div>
                   <span className="text-textSecondary text-sm font-bold uppercase tracking-wider block mb-2">O2 Beds</span>
                   <div className="text-5xl font-black text-textPrimary"><span className={o2Beds > 0 ? "text-emerald-400" : "text-red-500"}>{o2Beds}</span><span className="text-3xl text-textTertiary">/20</span></div>
                 </div>
                 <div className="flex gap-3">
                   <button onClick={() => setO2Beds(Math.max(0, o2Beds - 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">-</button>
                   <button onClick={() => setO2Beds(Math.min(20, o2Beds + 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">+</button>
                 </div>
              </div>
              ...
              <div className="bg-card border border-border rounded-3xl p-8 flex flex-col sm:flex-row sm:items-center justify-between shadow-lg gap-4">
                 <div>
                   <span className="text-textSecondary text-sm font-bold uppercase tracking-wider block mb-2">ER Ward</span>
                   <div className="text-5xl font-black text-textPrimary"><span className={erBeds > 0 ? "text-emerald-400" : "text-red-500"}>{erBeds}</span><span className="text-3xl text-textTertiary">/50</span></div>
                 </div>
                 <div className="flex gap-3">
                   <button onClick={() => setErBeds(Math.max(0, erBeds - 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">-</button>
                   <button onClick={() => setErBeds(Math.min(50, erBeds + 1))} className="w-16 h-16 bg-background border border-border rounded-2xl flex items-center justify-center hover:bg-surface active:scale-95 text-textSecondary text-3xl font-light transition-transform">+</button>
                 </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'stock' && (
          <div className="w-full max-w-3xl flex flex-col gap-6 animate-fade-in">
            <div className="flex justify-between items-center mb-2 border-b border-border pb-4">
              <h3 className="text-textPrimary font-bold flex items-center gap-2 text-2xl">🩸 Critical Inventory</h3>
            </div>
            
            <div className="space-y-6">
              <div className="bg-card rounded-3xl p-8 flex flex-col gap-6 border border-border shadow-xl">
                 <div className="flex justify-between items-start md:items-center flex-col md:flex-row gap-4">
                   <span className="text-2xl font-bold text-textPrimary">Universal O- Blood</span>
                   <button 
                 ...
                 <p className="text-textSecondary text-lg leading-relaxed bg-background p-4 rounded-xl border border-border">Required immediately for unknown trauma patient incoming.</p>
              </div>

              <div className="bg-card rounded-3xl p-8 flex flex-col gap-6 border border-border shadow-xl">
                 <div className="flex justify-between items-start md:items-center flex-col md:flex-row gap-4">
                   <span className="text-2xl font-bold text-textPrimary">AED Defibrillators</span>
                   <button 
                 ...
                 <p className="text-textSecondary text-lg leading-relaxed bg-background p-4 rounded-xl border border-border">Portable units available for dispatch to helipad.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'stats' && (
          <div className="w-full max-w-4xl flex flex-col gap-6 animate-fade-in">
            <h2 className="text-2xl font-bold text-textPrimary mb-2">Hospital Analytics</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {statsData.map((stat, i) => (
                <div key={i} className="bg-card rounded-3xl p-8 border border-border shadow-xl flex flex-col justify-between h-48 transition-transform hover:-translate-y-1">
                  <div className="flex justify-between items-start">
                    <div className="w-12 h-12 bg-background border border-border rounded-2xl flex items-center justify-center text-textSecondary">
                      {stat.icon}
                    </div>
                  ...
                  <div>
                    <h3 className="text-4xl font-black text-textPrimary mb-1">{stat.value}</h3>
                    <p className="text-textSecondary text-sm font-medium uppercase tracking-wider">{stat.label}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className={`fixed bottom-6 md:bottom-auto md:top-0 md:left-0 md:h-full w-[90%] left-[5%] md:left-0 bg-card backdrop-blur-xl border border-border md:border-y-0 md:border-l-0 md:border-r md:rounded-none rounded-full px-4 py-3 md:px-0 md:py-6 flex md:flex-col justify-between md:justify-start items-center gap-0 md:gap-4 shadow-2xl z-50 overflow-x-auto md:overflow-visible transition-transform duration-300 ${isSidebarExpanded ? 'md:w-64 md:items-start md:px-4 md:translate-x-0' : 'md:w-64 md:-translate-x-full'}`}>
        ...
        <div className="hidden md:flex w-full justify-end mb-4 pr-1">
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'radar' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'beds' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'stock' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
