    ...
    <div className="w-full min-h-[100dvh] bg-background text-textPrimary pb-[120px] relative">
      ...
      <div className="sticky top-0 z-40 w-full bg-background backdrop-blur-md border-b border-border p-4">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/family')} className="p-2 -ml-2 rounded-full hover:bg-surface transition-colors">
              <ArrowLeft size={20} />
            </button>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-textSecondary">Compare & Find Plans</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="relative p-2 bg-card rounded-xl border border-border hover:border-[#3D91FF]/50 transition-colors"
            >
              <Heart size={18} className={savedPlans.length > 0 ? "text-pink-400" : "text-textSecondary"} />
              {savedPlans.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-pink-500 text-textPrimary text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                  {savedPlans.length}
                </span>
              )}
            </button>
            <button 
              ...
              className="p-2 bg-card rounded-xl border border-border hover:border-emerald-500/50 transition-colors relative"
            >
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="flex overflow-x-auto gap-3 pb-2 scrollbar-hide">
          ...
          <button onClick={() => navigate('/insurance/saved')} className="bg-card border border-border text-textSecondary px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap">Saved Plans</button>
          <button onClick={() => navigate('/insurance/my-insurance')} className="bg-card border border-border text-textSecondary px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap">My Insurance</button>
        </div>

        {/* Discovery Form */}
        <div className="bg-gradient-to-br from-[#131F35] to-background rounded-3xl p-6 border border-border shadow-xl relative overflow-hidden">
          ...
          <form onSubmit={handleSearch} className="space-y-6 relative z-10">
            <div>
              <label className="block text-xs font-bold text-textSecondary uppercase mb-3">Who are you looking to insure?</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-bold text-textSecondary uppercase mb-2">Age of eldest member</label>
                <input type="number" placeholder="e.g. 45" className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#3D91FF]" />
              </div>
              <div>
                <label className="block text-xs font-bold text-textSecondary uppercase mb-2">City</label>
                <input type="text" placeholder="e.g. Mumbai" className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#3D91FF]" />
              </div>
              <div>
                <label className="block text-xs font-bold text-textSecondary uppercase mb-2">Preferred Coverage</label>
                <select 
                  value={coveragePref}
                  onChange={(e) => setCoveragePref(e.target.value)}
                  className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#3D91FF] appearance-none"
                >
        ...
        <div id="results-section" className="pt-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <select 
                ...
                className="flex-1 md:flex-none bg-card border border-border text-textSecondary px-4 py-2 rounded-xl text-sm font-bold focus:outline-none focus:border-[#3D91FF] appearance-none"
              >
          ...
          {/* Filters Panel */}
          {showFilters && (
            <div className="bg-card border border-border rounded-2xl p-4 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 animate-slide-up">
              <div>
                <label className="block text-[10px] font-bold text-textSecondary uppercase mb-2">Coverage Amount</label>
                <select value={filterCoverage} onChange={(e) => setFilterCoverage(e.target.value)} className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm text-textPrimary">
              ...
              <div>
                <label className="block text-[10px] font-bold text-textSecondary uppercase mb-2">Monthly Premium</label>
                <select value={filterPremium} onChange={(e) => setFilterPremium(e.target.value)} className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm text-textPrimary">
                  <option value="Any">Any</option>
              ...
              <div>
                <label className="block text-[10px] font-bold text-textSecondary uppercase mb-2">Insurance Type</label>
                <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm text-textPrimary">
                  <option value="Any">Any</option>
              ...
              <div className="md:col-span-3 flex justify-end gap-2 mt-2">
                <button 
                  ...
                  className="px-4 py-2 text-xs font-bold text-textSecondary hover:text-textPrimary"
                >
                  Reset
                </button>
                <button 
          ...
          {selectedComparePlans.length >= 2 && (
            <div className="bg-card border border-[#3D91FF]/30 rounded-3xl p-6 md:p-8 mb-8 shadow-[0_0_40px_rgba(61,145,255,0.05)] animate-in fade-in slide-in-from-bottom-4">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
                <h3 className="text-xl md:text-2xl font-bold text-textPrimary flex items-center gap-3">
              ...
              <div className="overflow-x-auto pb-4 scrollbar-hide">
                <div className="flex gap-4 min-w-max">
                  ...
                  <div className="flex flex-col gap-5 w-36 shrink-0 justify-end pt-[104px] pb-4 hidden md:flex">
                    <div className="text-[11px] font-bold text-textSecondary uppercase tracking-wider h-10 flex items-center border-b border-border">Premium</div>
                    <div className="text-[11px] font-bold text-textSecondary uppercase tracking-wider h-10 flex items-center border-b border-border">Coverage</div>
                    <div className="text-[11px] font-bold text-textSecondary uppercase tracking-wider h-10 flex items-center border-b border-border">Hospitals</div>
                    <div className="text-[11px] font-bold text-textSecondary uppercase tracking-wider h-10 flex items-center">Co-pay</div>
                  </div>
                  
                  {/* Plan Columns */}
                  {selectedComparePlans.map(plan => (
                    <div key={plan.id} className="w-[280px] bg-gradient-to-b from-background to-[#131F35] rounded-2xl border border-border p-5 shrink-0 flex flex-col relative overflow-hidden group hover:border-[#3D91FF]/50 transition-colors">
                      ...
                      <div className="mb-6 h-[80px]">
                        <span className="text-[10px] text-textSecondary bg-surface px-2.5 py-1 rounded-md mb-3 inline-block font-medium border border-border">{plan.provider}</span>
                        <h4 className="font-bold text-textPrimary text-base leading-tight line-clamp-2">{plan.planName}</h4>
                      </div>
                      
                      <div className="flex flex-col gap-5">
                        <div className="h-10 flex flex-col justify-center border-b border-border pb-2">
                          <span className="text-[10px] text-textTertiary font-bold uppercase md:hidden mb-1">Premium</span>
                          <span className="font-black text-emerald-400 text-lg">₹{plan.monthlyPremium.toLocaleString()}<span className="text-xs text-textTertiary font-medium">/mo</span></span>
                        </div>
                        <div className="h-10 flex flex-col justify-center border-b border-border pb-2">
                          <span className="text-[10px] text-textTertiary font-bold uppercase md:hidden mb-1">Coverage</span>
                          <span className="font-bold text-textPrimary">₹{(plan.coverage / 100000).toLocaleString()} Lakhs</span>
                        </div>
                        <div className="h-10 flex flex-col justify-center border-b border-border pb-2">
                          <span className="text-[10px] text-textTertiary font-bold uppercase md:hidden mb-1">Hospitals</span>
                        ...
                        <div className="h-10 flex flex-col justify-center">
                          <span className="text-[10px] text-textTertiary font-bold uppercase md:hidden mb-1">Co-pay</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPlans.length > 0 ? (
              ...
              <div className="col-span-full py-12 text-center text-textTertiary">
