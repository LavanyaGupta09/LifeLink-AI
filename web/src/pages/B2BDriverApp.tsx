...
const B2BDriverApp: React.FC = () => {
  ...
  return (
    <div className={`w-full min-h-screen bg-background text-textPrimary font-sans flex flex-col relative pb-[120px] md:pb-0 px-6 py-6 transition-all duration-300 ${isSidebarExpanded ? 'md:pl-72' : ''}`}>
      ...
      <header className="w-full flex justify-between items-center p-6 lg:px-10 lg:py-8 border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-3xl font-bold tracking-tight text-textPrimary flex items-center gap-3">
              CAD Dispatch
            ...
            <p className="text-textSecondary text-sm md:text-base mt-1">Unit: DL-01-AB-1234 • EMS Responder</p>
          </div>
        </div>
        <div className="flex gap-3">
           <button className="px-5 py-3 bg-card border border-border rounded-xl hover:bg-surface font-bold transition">Settings</button>
        </div>
      </header>
      ...
      <main className="w-full flex-1 p-6 lg:p-10">
        ...
        {activeTab === 'dispatch' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 h-full animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="w-full lg:col-span-4 flex flex-col gap-6 lg:gap-8 h-full">
              ...
              {/* Shift Toggle Panel */}
              <div className="bg-card border border-border rounded-3xl p-6 lg:p-8 shadow-xl flex flex-col gap-4">
                <h3 className="text-textSecondary font-bold tracking-wider uppercase text-sm">Duty Status</h3>
                <button 
                  ...
                  className={`w-full py-5 font-black text-xl flex justify-center items-center gap-3 rounded-2xl transition-all active:scale-95 ${
                    isOnline ? 'bg-emerald-600 text-textPrimary shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:bg-emerald-500' : 'bg-surface text-textSecondary border border-border hover:bg-surface'
                  }`}
                >
              ...
              {!isOnline && (
                <div className="bg-background border border-border rounded-3xl p-8 shadow-xl text-center flex-1 flex flex-col items-center justify-center">
                  <div className="w-24 h-24 bg-surface rounded-full flex items-center justify-center mb-6">
                    <Car size={40} className="text-textTertiary" />
                  </div>
                  <h2 className="text-2xl font-black text-textSecondary mb-2">You are Offline</h2>
                  <p className="text-textTertiary text-lg">Toggle your shift to ONLINE to connect to the CAD dispatch system.</p>
                </div>
              )}

              {isOnline && status === 'idle' && (
                <div className="bg-card border border-emerald-500/20 rounded-3xl p-8 shadow-xl text-center flex-1 flex flex-col items-center justify-center relative overflow-hidden">
                  ...
                  <h2 className="text-2xl font-bold mb-2 text-textPrimary relative z-10">Standing By</h2>
                  <p className="text-textSecondary text-lg relative z-10">Awaiting emergency dispatch...</p>
              ...
              {isOnline && status !== 'idle' && (
                <div className="bg-card border border-border rounded-3xl p-6 lg:p-8 shadow-xl flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-textPrimary">{scannedPatient?.name || 'Ravi Sharma (62M)'}</h3>
                      <p className="text-textSecondary text-lg">Suspected Cardiac Arrest</p>
                      {scannedPatient?.lifelink_token && (
                        <p className="text-emerald-400 text-xs mt-1 font-mono">Token: {scannedPatient.lifelink_token}</p>
                      )}
                    </div>
                    <button className="w-14 h-14 bg-surface rounded-full flex items-center justify-center text-emerald-400 hover:bg-surface transition-colors shadow-lg"><Phone size={24} /></button>
                  </div>
                    ...
                    <div className="mb-8 grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-bottom-4">
                      {scannedPatient.blood && (
                        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 flex flex-col">
                          ...
                          <span className="text-textPrimary font-black text-xl">{scannedPatient.blood}</span>
                        </div>
                      )}
                      {scannedPatient.allergies && scannedPatient.allergies.length > 0 && (
                        <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 flex flex-col justify-center">
                          ...
                          <span className="text-textPrimary font-semibold text-sm leading-tight line-clamp-2">{scannedPatient.allergies.join(', ')}</span>
                        </div>
                      )}
                      {scannedPatient.conditions && scannedPatient.conditions.length > 0 && (
                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 flex flex-col justify-center">
                          ...
                          <span className="text-textPrimary font-semibold text-sm leading-tight line-clamp-2">{scannedPatient.conditions.join(', ')}</span>
                        </div>
                      )}
                      {scannedPatient.meds && scannedPatient.meds.length > 0 && (
                        <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 flex flex-col justify-center">
                          ...
                          <span className="text-textPrimary font-semibold text-sm leading-tight line-clamp-2">{scannedPatient.meds.join(', ')}</span>
                        </div>
                      )}
                      {scannedPatient.contact && (
                        <div className="col-span-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 flex flex-col">
                          ...
                          <span className="text-textPrimary font-semibold text-sm">{scannedPatient.contact}</span>
                        </div>
                      )}
                    </div>
                  )}

                  <div className="bg-background rounded-2xl p-5 border border-border mb-8 flex gap-5">
                    <div className="w-12 h-12 bg-background rounded-full flex items-center justify-center shrink-0 shadow-inner"><AlertCircle size={24} className="text-emerald-500" /></div>
                    <div>
                      <p className="text-sm text-textTertiary font-bold uppercase tracking-wider mb-1">Pickup Location</p>
                      <p className="text-lg font-medium text-textPrimary">123 Safdarjung Enclave, New Delhi</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto">
                    {status === 'dispatched' ? (
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <button onClick={() => { setStatus('idle'); setScannedPatient(null); }} className="w-full bg-surface hover:bg-surface text-textPrimary font-black text-lg py-5 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-3 border border-border shadow-lg">
            ...
            <div className="w-full lg:col-span-8 bg-card border border-border rounded-3xl shadow-xl overflow-hidden min-h-[500px] relative flex flex-col">
               {isOnline && status !== 'idle' ? (
                 <>
                   <div className="absolute top-6 left-6 z-10 flex gap-4">
                     <div className="bg-card backdrop-blur-md border border-border px-6 py-4 rounded-2xl shadow-2xl flex flex-col items-center">
                       ...
                       <p className="text-3xl font-black text-textPrimary">{etaMins} <span className="text-lg text-textSecondary">min</span></p>
                     </div>
                     <div className="bg-card backdrop-blur-md border border-border px-6 py-4 rounded-2xl shadow-2xl flex flex-col items-center">
                       <div className="flex items-center gap-2 text-textSecondary font-black mb-1 text-sm tracking-wider uppercase"><Navigation size={16} /> Dist</div>
                       <p className="text-3xl font-black text-textPrimary">{distanceKm} <span className="text-lg text-textSecondary">km</span></p>
                     </div>
                   </div>
                 ...
                 <div className="w-full h-full flex items-center justify-center bg-background flex-col">
                   ...
                   <h2 className="text-textTertiary font-bold text-xl">Map Offline</h2>
                   <p className="text-textTertiary">Go online and accept a dispatch to view route.</p>
                 </div>
               )}
            </div>
        ...
        {activeTab === 'ledger' && (
          <div className="w-full animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-textPrimary">Dispatch Ledger</h2>
              <div className="bg-card border border-border rounded-lg p-1 flex items-center">
                 <button className="px-4 py-1.5 rounded-md bg-surface text-textPrimary text-sm font-medium">Today</button>
                 <button className="px-4 py-1.5 rounded-md text-textSecondary text-sm font-medium hover:text-textPrimary">This Week</button>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-background border-b border-border">
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary">Date/Time</th>
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary">Patient</th>
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary">Destination</th>
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary">Distance</th>
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary">Status</th>
                    <th className="p-5 text-xs font-bold uppercase tracking-wider text-textSecondary text-right">Action</th>
                  </tr>
                </thead>
                  ...
                  {[1, 2, 3, 4, 5].map((_, i) => (
                    <tr key={i} className="hover:bg-surface transition-colors">
                      <td className="p-5 text-sm text-textSecondary">
                        <div className="font-bold text-textPrimary">Aug 1{i}, 2026</div>
                        <div className="text-textTertiary">14:{i}0 PM</div>
                      </td>
                      <td className="p-5 text-sm font-medium text-textPrimary flex items-center gap-3">
                      ...
                      <td className="p-5 text-sm text-textSecondary">Max Super Speciality Hospital</td>
                      <td className="p-5 text-sm text-textSecondary font-bold">{12 + i}.4 km</td>
        ...
        {activeTab === 'maintenance' && (
          <div className="w-full animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-2xl font-bold text-textPrimary mb-8">Fleet Telemetry</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center justify-center shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-textSecondary font-bold tracking-widest uppercase text-xs mb-1">Fuel Level</h3>
                <p className="text-3xl font-black text-textPrimary">78%</p>
              </div>
              <div className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center justify-center shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-textSecondary font-bold tracking-widest uppercase text-xs mb-1">Odometer</h3>
                <p className="text-3xl font-black text-textPrimary">42,105 <span className="text-lg text-textTertiary">km</span></p>
              </div>
              <div className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center justify-center shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-textSecondary font-bold tracking-widest uppercase text-xs mb-1">Battery Health</h3>
                <p className="text-3xl font-black text-textPrimary">94%</p>
              </div>
            </div>

            <div className="bg-card border border-border rounded-3xl p-8 shadow-xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-textPrimary flex items-center gap-2"><Wrench size={20} className="text-textSecondary" /> Vehicle Health Status</h3>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-5 bg-background rounded-2xl border border-border">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-textPrimary font-bold">Engine Diagnostics</h4>
                      <p className="text-sm text-textSecondary">Last checked: Today, 08:00 AM</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between p-5 bg-background rounded-2xl border border-border">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400"><CheckCircle size={20} /></div>
                    <div>
                      <h4 className="text-textPrimary font-bold">Tire Pressure Monitor</h4>
                      <p className="text-sm text-textSecondary">32 PSI average across all tires</p>
                    </div>
                  </div>
                  <span className="text-emerald-400 font-bold text-sm">OK</span>
                </div>
                <div className="flex items-center justify-between p-5 bg-background rounded-2xl border border-rose-500/30 bg-rose-500/5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-textPrimary font-bold text-rose-400">Oxygen Cylinder Capacity</h4>
                      <p className="text-sm text-textSecondary">Main cylinder at 15% - requires refill</p>
                    </div>
                  </div>
                  <button className="px-4 py-2 bg-rose-500/10 text-rose-400 hover:bg-rose-500 hover:text-textPrimary transition-colors rounded-lg text-xs font-bold uppercase tracking-wider border border-rose-500/20">Request Refill</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className={`fixed bottom-6 md:bottom-auto md:top-0 md:left-0 md:h-full w-[90%] left-[5%] md:left-0 bg-card backdrop-blur-xl border border-border md:border-y-0 md:border-l-0 md:border-r md:rounded-none rounded-full px-4 py-3 md:px-0 md:py-6 flex md:flex-col justify-between md:justify-start items-center gap-0 md:gap-4 shadow-2xl z-50 overflow-x-auto md:overflow-visible transition-transform duration-300 ${isSidebarExpanded ? 'md:w-64 md:items-start md:px-4 md:translate-x-0' : 'md:w-64 md:-translate-x-full'}`}>
        ...
        <div className="hidden md:flex w-full justify-end mb-4 pr-1">
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'dispatch' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'ledger' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
         ...
         <button 
           ...
           className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'maintenance' ? 'text-emerald-400 md:bg-emerald-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`}
