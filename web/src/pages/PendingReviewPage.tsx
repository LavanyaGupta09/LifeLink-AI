...
const PendingReviewPage: React.FC = () => {
  ...
  return (
    <div className="bg-background flex flex-col items-center justify-center p-6 font-sans text-textPrimary w-full min-h-screen px-6 py-6 ">
      {/* Elevated Card Container */}
      <div className="max-w-md md:max-w-2xl w-full bg-surface rounded-3xl shadow-2xl border border-border p-8 text-center relative overflow-hidden">
        ...
        {/* Main Title */}
        <h1 className="text-3xl font-black mb-8 tracking-tight text-textPrimary relative z-10">
          Credentials Under Review
        </h1>
        
        {/* Dynamic Data Blocks */}
        <div className="space-y-6 text-left relative z-10">
          <div className="bg-background rounded-2xl p-5 border border-border flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-black tracking-widest text-textSecondary uppercase mb-1">Provider Account</p>
              <p className="font-bold text-lg text-textPrimary leading-tight mb-0.5">{user?.fullName || 'Enterprise Provider'}</p>
              <p className="text-sky-400 font-medium text-sm">{user?.role?.replace('_', ' ').toUpperCase() || 'HOSPITAL ADMIN'}</p>
            </div>
          </div>
          
          <div className="bg-background rounded-2xl p-5 border border-border">
            <p className="text-[10px] font-black tracking-widest text-textSecondary uppercase mb-1">Verification Status</p>
        ...
        <button 
          ...
          className="mt-8 flex items-center justify-center gap-2 w-full bg-background hover:bg-surface text-textPrimary font-bold py-4 px-6 rounded-full border border-sky-500/30 hover:border-sky-500/60 transition-all duration-300 relative z-10 hover:shadow-[0_0_20px_rgba(14,165,233,0.15)]"
        >
