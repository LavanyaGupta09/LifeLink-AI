    ...
    <div className="p-4 md:p-8 w-full max-w-6xl mx-auto flex-1 animate-in fade-in zoom-in-95 duration-200">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        <div>
          ...
          <p className="text-textSecondary mt-1 text-sm md:text-base">Manage your upcoming consultations and visits.</p>
        </div>
        <div className="w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
          <div className="bg-card border border-border rounded-xl p-1 flex items-center gap-1 min-w-max">
             <button className="px-4 py-2 rounded-lg bg-surface text-textPrimary text-sm font-bold shadow-md">Today</button>
             <button className="px-4 py-2 rounded-lg text-textSecondary hover:text-textPrimary text-sm font-bold transition-colors">Tomorrow</button>
             <button className="px-4 py-2 rounded-lg text-textSecondary hover:text-textPrimary text-sm font-bold transition-colors flex items-center gap-2">
      ...
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-bold text-textPrimary">Thursday, Aug 19, 2026</h3>
        <button className="flex items-center gap-2 text-textSecondary hover:text-textPrimary font-bold text-sm bg-card px-4 py-2 rounded-lg border border-border transition-colors">
          <Filter size={14} /> Filter
        </button>
      </div>

      <div className="space-y-4 relative">
        <div className="absolute left-10 top-4 bottom-4 w-px bg-surface z-0 hidden md:block"></div>
        {schedule.map((apt, i) => (
          <div key={i} className="bg-card border border-border rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between hover:border-blue-500/30 transition-colors relative z-10">
            ...
            <div className="flex items-start md:items-center gap-4 md:gap-6 mb-4 md:mb-0">
              ...
              <div className="flex items-center gap-3 md:gap-4 w-full">
                ...
                <div className="flex-1">
                  <h3 className="text-base md:text-lg font-bold text-textPrimary leading-tight">{apt.name}</h3>
                  <p className="text-textSecondary text-xs md:text-sm mt-1">{apt.type}</p>
                </div>
                <div className="md:hidden text-right">
                   <span className="text-sm font-bold text-textPrimary block">{apt.time}</span>
                   <span className={`text-[10px] font-bold px-2 py-0.5 rounded mt-1 inline-block ${apt.status === 'Completed' ? 'bg-surface text-textSecondary' : 'bg-blue-500/20 text-blue-400'}`}>{apt.status}</span>
                </div>
              </div>
            </div>

            <div className="flex flex-wrap md:flex-nowrap items-center gap-4 md:gap-8 ml-0 md:ml-6">
              <div className="flex items-center gap-2 text-textSecondary bg-background px-3 py-1.5 rounded-lg border border-border text-sm">
              ...
              {apt.status === 'Upcoming' ? (
                ...
                <button className="ml-auto md:ml-0 px-6 py-2.5 bg-surface text-textSecondary rounded-xl font-bold text-sm border border-border cursor-not-allowed">
                  Completed
                </button>
              )}
            </div>
