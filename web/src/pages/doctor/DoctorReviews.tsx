  ...
  return (
    <div className="p-4 md:p-8 w-full max-w-7xl mx-auto flex-1 animate-in fade-in zoom-in-95 duration-200">
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-textSecondary mt-1 text-sm md:text-base">Read feedback from your past consultations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-1">
          <div className="bg-card border border-border rounded-3xl p-6 text-center">
            <h3 className="text-sm font-bold text-textSecondary uppercase tracking-widest mb-4">Overall Rating</h3>
            <div className="text-5xl font-black text-textPrimary mb-2">4.8</div>
            <div className="flex justify-center text-amber-500 mb-2">
              <Star size={24} fill="currentColor" />
              <Star size={24} fill="currentColor" />
              <Star size={24} fill="currentColor" />
              <Star size={24} fill="currentColor" />
              <Star size={24} className="text-textTertiary" />
            </div>
            <p className="text-textSecondary text-sm">Based on 124 reviews</p>
            
            <div className="mt-8 space-y-3">
              {[5, 4, 3, 2, 1].map(num => (
                <div key={num} className="flex items-center gap-3">
                  <span className="text-textSecondary text-xs w-2">{num}</span>
            ...
            <div key={i} className="bg-card border border-border rounded-2xl p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex gap-4 items-center">
                  <img src={`https://i.pravatar.cc/100?img=${i + 5}`} alt={r.name} className="w-10 h-10 rounded-full border border-border" />
                  <div>
                    <h4 className="text-textPrimary font-bold">{r.name}</h4>
                    <p className="text-textTertiary text-xs">{r.date}</p>
                  </div>
                </div>
                <div className="flex text-amber-500">
                  {[...Array(5)].map((_, idx) => (
                    <Star key={idx} size={14} fill={idx < r.rating ? 'currentColor' : 'none'} className={idx >= r.rating ? 'text-textTertiary' : ''} />
                  ))}
                </div>
              </div>
              <p className="text-textSecondary text-sm">{r.comment}</p>
              
              <div className="mt-4 pt-4 border-t border-border flex gap-4">
                <button className="flex items-center gap-1.5 text-textSecondary hover:text-textPrimary text-xs font-bold transition-colors">
