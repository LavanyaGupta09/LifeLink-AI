    ...
    <div className="p-4 md:p-8 w-full max-w-6xl mx-auto flex-1 animate-in fade-in zoom-in-95 duration-200">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-textSecondary mt-1 text-sm md:text-base">Patients currently waiting in your digital queue.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {activeConsultations.map((patient, i) => (
          <div key={i} className="bg-card border border-border rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between hover:border-emerald-500/30 transition-all group">
            ...
            <div className="flex flex-col sm:flex-row items-start gap-4 md:gap-6 mb-4 md:mb-0 w-full md:w-auto">
              <div className="relative shrink-0 flex gap-4 w-full sm:w-auto items-center sm:items-start">
                <img src={`https://i.pravatar.cc/100?img=${i + 20}`} alt={patient.name} className="w-14 h-14 md:w-16 md:h-16 rounded-2xl object-cover border-2 border-border group-hover:border-emerald-500 transition-colors" />
                <div className="absolute -bottom-2 left-10 md:left-12 bg-background rounded-full p-1 border border-border">
              ...
              <div className="w-full sm:w-auto">
                <h3 className="hidden sm:flex text-xl font-bold text-textPrimary mb-1 items-center gap-3">
                  {patient.name}
                  <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border ${getTriageColor(patient.triage)}`}>
                    {patient.triage}
                  </span>
                </h3>
                <p className="text-textSecondary text-xs md:text-sm mb-3">{patient.age}y • {patient.gender} • Blood: {patient.blood_group}</p>
            ...
            <div className="flex flex-col items-start md:items-end gap-4 ml-0 md:ml-6 mt-4 md:mt-0 w-full md:w-auto border-t border-border md:border-t-0 pt-4 md:pt-0">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                <button className="flex-1 md:flex-none px-4 py-3 bg-surface hover:bg-surface text-textSecondary transition-colors rounded-xl font-bold text-sm flex items-center justify-center gap-2">
