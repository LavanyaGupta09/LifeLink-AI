...
export const AdminActivity: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">System Activity</h1>
        <p className="text-textSecondary mt-1 text-sm">Real-time feed of all actions across the platform.</p>
      </div>
        ...
        {['All', 'Provider', 'User', 'Security', 'System'].map(f => (
          <button 
            ...
            className={`px-4 py-2 text-xs font-bold rounded-lg border ${
              filter === f ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300' : 'bg-background border-border text-textSecondary'
            }`}
          >
            {f}
          </button>
      ...
      <div className="bg-background border border-border rounded-xl p-5 flex flex-col gap-6">
        {filtered.map(act => (
          <div key={act.id} className="flex gap-4 border-b border-border pb-4 last:border-0 last:pb-0">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-textPrimary">{act.action}</p>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="w-1 h-1 rounded-full bg-surface"></span>
                <span className="text-[10px] text-textTertiary">{new Date(act.time).toLocaleString()}</span>
              </div>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="text-center py-8 text-textTertiary text-sm">No activities found for this filter.</div>
        )}
      </div>
    </div>
  );
...
export const AdminSecurity: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">Security Center</h1>
        <p className="text-textSecondary mt-1 text-sm">Monitor and resolve system security incidents.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        <div className="bg-background border border-border rounded-xl p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-textSecondary uppercase">System Status</p>
      ...
      <h2 className="text-sm font-bold text-textPrimary uppercase mt-4">Active Alerts ({activeAlerts.length})</h2>
      <div className="flex flex-col gap-3">
          ...
          <div key={alert.id} className="bg-red-500/5 border border-red-500/20 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-textSecondary">{new Date(alert.time).toLocaleString()}</span>
              </div>
            ...
            <button 
              ...
              className="px-4 py-2 bg-surface hover:bg-emerald-500/20 hover:text-emerald-400 text-textSecondary text-sm font-bold rounded-lg transition-colors shrink-0 flex items-center justify-center gap-2"
            >
        ...
        {activeAlerts.length === 0 && (
          <div className="bg-background border border-border rounded-xl p-8 text-center flex flex-col items-center justify-center">
            <ShieldCheck size={40} className="text-emerald-500/50 mb-3" />
            <p className="text-textPrimary font-bold">No active security alerts</p>
            <p className="text-textSecondary text-sm">The system is currently secure.</p>
          </div>
        )}
      </div>
      ...
      {resolvedAlerts.length > 0 && (
        <>
          <h2 className="text-sm font-bold text-textPrimary uppercase mt-6">Recent Resolved Alerts</h2>
          <div className="flex flex-col gap-3">
            {resolvedAlerts.slice(0, 5).map(alert => (
              <div key={alert.id} className="bg-background border border-border rounded-xl p-4 flex items-center justify-between opacity-70">
                <div>
                  <h3 className="text-sm font-bold text-textPrimary line-through">{alert.title}</h3>
                  <p className="text-xs text-textSecondary mt-0.5">{alert.description}</p>
                </div>
...
export const AdminSystemHealth: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">System Health</h1>
        <p className="text-textSecondary mt-1 text-sm">Real-time status of LifeLink microservices.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map(srv => (
          <div key={srv.name} className="bg-background border border-border rounded-xl p-5">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-sm font-bold text-textPrimary">{srv.name}</h3>
            ...
            <div className="flex justify-between items-center text-xs mt-2">
              <span className="text-textSecondary">Uptime (30d)</span>
              <span className="text-textSecondary font-mono">{srv.uptime}</span>
            </div>
            <div className="flex justify-between items-center text-xs mt-2">
              <span className="text-textSecondary">Avg Latency</span>
              <span className="text-textSecondary font-mono">{srv.latency}</span>
            </div>
          </div>
        ))}
      </div>
...
export const AdminReports: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">Reports & Complaints</h1>
        <p className="text-textSecondary mt-1 text-sm">Manage user-submitted reports and network issues.</p>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background/50 border-b border-border">
                ...
                <th className="px-5 py-4 text-xs font-bold text-textSecondary uppercase text-right">Status / Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {reports.map(rep => (
                <tr key={rep.id} className="hover:bg-surface">
                  <td className="px-5 py-4">
                    <p className="text-sm font-bold text-textPrimary">{rep.id}</p>
                    <p className="text-xs text-textSecondary mt-0.5">By: {rep.reporter}</p>
                    <p className="text-[10px] text-textTertiary">{rep.date}</p>
                  </td>
                  <td className="px-5 py-4">
                    <p className="text-sm text-textSecondary">{rep.entity}</p>
                    <p className="text-xs text-textTertiary mt-0.5">{rep.category}</p>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="inline-flex items-center">
                      <select 
                        ...
                        className={`bg-background border border-border rounded-lg px-3 py-1.5 text-xs font-bold focus:outline-none focus:border-indigo-500 ${
...
export const AdminAnalytics = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">Platform Analytics</h1>
        <p className="text-textSecondary mt-1 text-sm">Visualize user growth and provider distributions.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-background border border-border rounded-xl p-6 flex flex-col">
          <h2 className="text-sm font-bold text-textPrimary uppercase tracking-wider mb-6">User Registration Growth (2026)</h2>
          <div className="flex-1 flex items-end gap-2 sm:gap-4 h-64 mt-auto">
            {userGrowth.map((data, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-2 group">
                <div className="w-full relative bg-surface rounded-t-sm h-full flex items-end">
                  <div 
                  ...
                  >
                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-background text-textPrimary text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                      {data.value}k Users
                    </div>
                  </div>
                </div>
                <span className="text-xs text-textSecondary font-medium">{data.month}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-background border border-border rounded-xl p-6 flex flex-col">
          <h2 className="text-sm font-bold text-textPrimary uppercase tracking-wider mb-6">Provider Distribution</h2>
              ...
              return (
                <div key={idx} className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium text-textSecondary">{item.type}</span>
                    <span className="text-xs font-bold text-textPrimary">{percentage}% <span className="text-textTertiary font-normal">({item.count})</span></span>
                  </div>
    ...
    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full pb-20 md:pb-0">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-textSecondary mt-1 text-sm">View system alerts and platform updates.</p>
        </div>
      ...
      <div className="bg-background border border-border rounded-xl overflow-hidden flex flex-col">
        {notifications.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-center">
            <Bell size={32} className="text-textTertiary mb-4" />
            <p className="text-textPrimary font-bold">You're all caught up!</p>
            <p className="text-textSecondary text-sm mt-1">No new notifications to display.</p>
          </div>
        ) : (
              ...
              <div key={notif.id} className={`p-4 flex gap-4 transition-colors hover:bg-surface ${notif.read ? 'opacity-70' : 'bg-card'}`}>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className={`text-sm font-bold ${notif.read ? 'text-textSecondary' : 'text-textPrimary'}`}>{notif.title}</h3>
                    <span className="text-[10px] text-textTertiary font-medium whitespace-nowrap ml-2">{notif.time}</span>
                  </div>
                  <p className="text-sm text-textSecondary mt-1">{notif.message}</p>
                </div>
                <button 
                  onClick={() => deleteNotification(notif.id)}
                  className="shrink-0 p-2 text-textTertiary hover:text-red-400 transition-colors h-fit rounded-lg hover:bg-red-500/10"
                >
...
export const AdminProfile = () => {
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">Admin Profile</h1>
        <p className="text-textSecondary mt-1 text-sm">Manage your account details and active sessions.</p>
      </div>

      <div className="bg-background border border-border rounded-xl p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start">
        <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/20 shrink-0">
          <span className="text-4xl font-black text-textPrimary">A</span>
        </div>
        <div className="flex-1">
          <h2 className="text-xl font-bold text-textPrimary">Administrator</h2>
          <p className="text-indigo-400 text-sm font-medium mt-1">Super Admin Role</p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            <div>
              <p className="text-[10px] font-bold text-textTertiary uppercase tracking-wider">Email Address</p>
              <p className="text-sm text-textSecondary mt-1">admin@lifelink.ai</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-textTertiary uppercase tracking-wider">Phone Number</p>
              <p className="text-sm text-textSecondary mt-1">+91 98765 43210</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-textTertiary uppercase tracking-wider">Location</p>
              <p className="text-sm text-textSecondary mt-1">HQ - New Delhi, India</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-textTertiary uppercase tracking-wider">Member Since</p>
              <p className="text-sm text-textSecondary mt-1">Jan 2026</p>
            </div>
          </div>

          <div className="flex gap-3 mt-8">
            <button className="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-textPrimary text-sm font-bold rounded-lg transition-colors shadow">
              Edit Profile
            </button>
            <button className="px-4 py-2 bg-background border border-border hover:border-border text-textPrimary text-sm font-bold rounded-lg transition-colors">
              Change Password
            </button>
          </div>
        </div>
      ...
      <div className="bg-background border border-border rounded-xl overflow-hidden divide-y divide-slate-800/50">
        <div className="p-4 flex justify-between items-center bg-card">
          <div>
            ...
            <p className="text-xs text-textSecondary mt-1">IP: 192.168.1.100 • Last active: Just now</p>
          </div>
        </div>
        <div className="p-4 flex justify-between items-center">
          <div>
            <p className="text-sm font-bold text-textSecondary">iOS • Safari</p>
            <p className="text-xs text-textSecondary mt-1">IP: 10.0.0.45 • Last active: 2 hours ago</p>
          </div>
...
export const AdminSettings = () => {
  ...
  const ToggleSwitch = ({ checked, onChange, label, description, critical = false }: any) => (
    <div className="flex items-center justify-between p-4 bg-background rounded-xl border border-border hover:border-border transition-colors">
      <div className="flex-1 pr-4">
        <h3 className="text-sm font-bold text-textPrimary">{label}</h3>
      ...
      <button 
        ...
        className={`w-12 h-6 rounded-full relative transition-colors duration-200 ease-in-out shrink-0 focus:outline-none ${
          checked ? (critical ? 'bg-red-500' : 'bg-indigo-500') : 'bg-surface'
        }`}
      >
        <span 
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">System Settings</h1>
        <p className="text-textSecondary mt-1 text-sm">Configure global platform toggles and feature flags.</p>
      </div>
