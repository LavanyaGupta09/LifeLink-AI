...
const AdminUsers: React.FC = () => {
  ...
  const getStatusBadge = (status: UserStatus) => {
    switch (status) {
      ...
      case 'inactive':
        return <span className="px-2.5 py-1 bg-slate-500/20 text-textSecondary text-[10px] font-bold uppercase tracking-wider rounded-md">Inactive</span>;
      case 'suspended':
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full pb-20 md:pb-0 relative">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-textPrimary tracking-tight">User Management</h1>
        <p className="text-textSecondary mt-1 text-sm">Manage patient and system user accounts.</p>
      </div>

      {/* Tabs & Search */}
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        <div className="flex overflow-x-auto custom-scrollbar gap-2 border-b border-border pb-px w-full md:w-auto">
          {[
      ...
      <div className="bg-background border border-border rounded-xl overflow-hidden">
              ...
              {filteredUsers.map(user => (
                <tr key={user.id} className="hover:bg-surface transition-colors group">
                  <td className="px-5 py-4">
                    ...
                    <p className="text-xs text-textSecondary mt-0.5">{user.email} • {user.phone}</p>
                  </td>
                  <td className="px-5 py-4">
                    <p className="text-sm text-textSecondary font-medium">{user.role}</p>
                    <p className="text-[10px] text-textTertiary font-mono mt-0.5">{user.id}</p>
                  </td>
                  <td className="px-5 py-4">
                    <p className="text-sm text-textSecondary">{new Date(user.registrationDate).toLocaleDateString()}</p>
                    <p className="text-[10px] text-textTertiary mt-0.5">Last active: {new Date(user.lastActive).toLocaleDateString()}</p>
                  </td>
                  <td className="px-5 py-4">
        ...
        <div className="md:hidden flex flex-col divide-y divide-slate-800/50">
          {filteredUsers.map(user => (
            <div key={user.id} className="p-4 flex flex-col gap-3 hover:bg-surface transition-colors">
              <div className="flex justify-between items-start gap-2">
                <div>
                  <h3 className="text-base font-bold text-textPrimary">{user.name}</h3>
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-xs text-textSecondary bg-surface px-2 py-0.5 rounded-md">{user.role}</span>
                    <span className="text-xs text-textTertiary">{user.email}</span>
                  </div>
                </div>
                {getStatusBadge(user.status)}
              </div>
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-border">
                <div className="text-[10px] text-textTertiary">
                  ID: <span className="text-textSecondary font-mono">{user.id}</span>
                </div>
                {user.status === 'suspended' ? (
                  <button 
                    onClick={() => setShowConfirmModal({ id: user.id, name: user.name, action: 'active' })}
        ...
        {filteredUsers.length === 0 && (
          <div className="p-12 flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 bg-surface rounded-full flex items-center justify-center mb-4">
              <Users size={24} className="text-textTertiary" />
            </div>
            <h3 className="text-lg font-bold text-textPrimary">No Users Found</h3>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="bg-background border border-border rounded-2xl p-6 max-w-md w-full relative z-10 animate-scale-up shadow-2xl">
            <h2 className="text-xl font-bold text-textPrimary mb-2 flex items-center gap-2">
              <ShieldAlert className={showConfirmModal.action === 'suspended' ? 'text-red-500' : 'text-emerald-500'} size={24} />
              Confirm Action
            </h2>
            <p className="text-textSecondary text-sm mb-6">
              Are you sure you want to {showConfirmModal.action === 'suspended' ? 'suspend' : 'reactivate'} the account for <strong className="text-textPrimary">{showConfirmModal.name}</strong>?
