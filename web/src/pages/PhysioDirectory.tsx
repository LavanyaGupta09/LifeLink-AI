    ...
    <div className="w-full min-h-screen bg-background text-textPrimary font-sans flex flex-col pb-[120px] px-6 py-6">
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input 
            ...
            className="w-full bg-card border border-border rounded-2xl py-3 pl-11 pr-4 text-sm text-textPrimary focus:outline-none focus:border-emerald-500/50"
          />
        </div>
        <button className="w-12 h-12 rounded-2xl bg-card border border-border flex items-center justify-center text-textSecondary">
          <Filter size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        {DIRECTORY.filter(d => d.name.toLowerCase().includes(search.toLowerCase()) || d.spec.toLowerCase().includes(search.toLowerCase())).map(p => (
          <div key={p.id} className="bg-card border border-border rounded-3xl p-5 shadow-xl relative group">
            <div className="flex gap-4 mb-4">
              ...
              <div className="flex-1 pt-1">
                ...
                <div className="flex flex-col gap-1 text-[11px] text-textSecondary font-medium">
            ...
            <div className="flex items-center justify-between border-t border-border pt-4">
              <div>
                <p className="text-[10px] font-bold text-textTertiary uppercase">Per Session</p>
