      ...
      <div className="w-full min-h-screen bg-[#040814] text-white pb-32">
        <div className="w-full px-4 py-4 flex items-center gap-3 sticky top-0 bg-[#040814]/95 backdrop-blur-sm z-30 border-b border-slate-800/50">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">ASHA Visit</p>
          </div>
        </div>
        ...
        <div className="px-4 py-6 max-w-lg mx-auto">
          ...
          {/* Check Items */}
          <h3 className="text-base font-bold mb-3 text-slate-200">Kya-kya check karegi?</h3>
          <div className="grid grid-cols-2 gap-3 mb-6">
            {checkItems.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className="bg-[#0B1221] border border-slate-800 rounded-2xl p-5 flex flex-col items-center gap-3 text-center"
                >
                  <div
      ...
      <div className="w-full min-h-screen bg-[#040814] text-white pb-32">
        <div className="w-full px-4 py-4 flex items-center gap-3 sticky top-0 bg-[#040814]/95 backdrop-blur-sm z-30 border-b border-slate-800/50">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Patient ki details bharein</p>
          </div>
        </div>
        ...
        <div className="px-4 py-6 max-w-lg mx-auto">
          <div className="flex flex-col gap-4">
            {/* Patient Name */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">👤 Patient ka Naam</label>
              <input
                type="text"
                value={form.patientName}
                onChange={(e) => setForm({ ...form, patientName: e.target.value })}
                placeholder="Naam likhein..."
                className="w-full bg-[#0B1221] border border-slate-700 rounded-xl py-4 px-4 text-base text-white placeholder-slate-600 focus:outline-none focus:border-[#3B82F6] transition-colors"
              />
            </div>

            {/* Temperature */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">🌡 Bukhar / Temperature (°F)</label>
              <input
                type="number"
                value={form.temperature}
                onChange={(e) => setForm({ ...form, temperature: e.target.value })}
                placeholder="98.6"
                className="w-full bg-[#0B1221] border border-slate-700 rounded-xl py-4 px-4 text-base text-white placeholder-slate-600 focus:outline-none focus:border-[#FF4757] transition-colors"
              />
            </div>

            {/* BP */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">🩺 BP (Blood Pressure)</label>
              <input
                type="text"
            ...
            {/* Breathing */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">🫁 Saans lene mein dikkat</label>
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${
                      form.breathingDifficulty === level
                        ...
                        : 'bg-[#0B1221] border border-slate-700 text-slate-400'
                    }`}
                  >
            ...
            {/* Heart Rate */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">❤️ Dhadkan (Heart Rate)</label>
              <input
                type="number"
                value={form.heartRate}
                onChange={(e) => setForm({ ...form, heartRate: e.target.value })}
                placeholder="72"
                className="w-full bg-[#0B1221] border border-slate-700 rounded-xl py-4 px-4 text-base text-white placeholder-slate-600 focus:outline-none focus:border-[#8B5CF6] transition-colors"
              />
            </div>

            {/* Condition */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">📋 Patient ki halat</label>
              <input
                type="text"
                value={form.condition}
                onChange={(e) => setForm({ ...form, condition: e.target.value })}
                placeholder="Kya dikkat hai..."
                className="w-full bg-[#0B1221] border border-slate-700 rounded-xl py-4 px-4 text-base text-white placeholder-slate-600 focus:outline-none focus:border-[#00C9A7] transition-colors"
              />
            </div>

            {/* Notes */}
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">📝 Kya salah di?</label>
              <textarea
                value={form.notes}
                onChange={(e) => setForm({ ...form, notes: e.target.value })}
                placeholder="Dawai, salah, ya koi aur baat..."
                rows={3}
                className="w-full bg-[#0B1221] border border-slate-700 rounded-xl py-4 px-4 text-base text-white placeholder-slate-600 focus:outline-none focus:border-[#00C9A7] transition-colors resize-none"
              />
            </div>
    ...
    <div className="w-full min-h-screen bg-[#040814] text-white pb-32">
      ...
      <div className="px-4 py-12 max-w-lg mx-auto text-center">
        ...
        <p className="text-slate-400 text-sm mb-2">
          <strong className="text-white">{form.patientName || 'Patient'}</strong> ki jaanch record ho gayi hai.
        </p>
        {parseFloat(form.temperature) >= 103 || form.breathingDifficulty === 'severe' ? (
          <div className="bg-[#FF4757]/10 border border-[#FF4757]/30 rounded-xl p-4 mt-4 mb-6">
            ...
            <p className="text-slate-300 text-sm">Patient ki halat gambhir lag rahi hai.</p>
            <button
