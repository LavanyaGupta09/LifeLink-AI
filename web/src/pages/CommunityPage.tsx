          ...
          <>
            ...
            <div className="flex gap-4">
              <div className="relative flex-1">
                <Search size={18} className="absolute left-4 top-1/2 transform -translate-y-1/2 text-textTertiary" />
                <input 
                  type="text" 
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search discussions..." 
                  className="w-full bg-card border border-border rounded-2xl py-3 pl-12 pr-4 focus:outline-none focus:border-[#8B5CF6] transition-colors text-textPrimary"
                />
              </div>
              <button 
                onClick={() => setActiveTab('ai')}
            ...
            <div className="flex flex-col gap-4 pb-10">
              {filteredPosts.length === 0 ? (
                <div className="text-center py-12 bg-card rounded-3xl border border-border">
                  <MessageCircle size={40} className="mx-auto text-textTertiary mb-4" />
                  <h3 className="text-lg font-bold text-textSecondary">No discussions found</h3>
                  <p className="text-textTertiary text-sm">Try adjusting your filters or ask a new question.</p>
                </div>
              ) : (
                filteredPosts.map(post => (
                  <div key={post.id} className="bg-card border border-border rounded-3xl p-6 shadow-xl transition-all hover:border-border group">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${post.authorRole === 'Verified Doctor' ? 'bg-[#00C9A7]/20 text-[#00C9A7]' : 'bg-surface text-textPrimary'}`}>
                          {post.avatar}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-textPrimary text-sm">{post.author}</span>
                          ...
                          <span className="text-xs text-textTertiary font-medium">{post.timeAgo} • {post.category}</span>
                        </div>
                      </div>
                      <button className="text-textTertiary hover:text-textSecondary transition-colors">
    ...
    <div className="flex flex-col gap-6">
      <form onSubmit={handleSearch} className="relative">
        ...
        <input 
          ...
          className="w-full bg-card border-2 border-border rounded-2xl py-5 pl-14 pr-32 focus:outline-none focus:border-[#8B5CF6] transition-colors text-lg text-textPrimary shadow-xl"
          disabled={isSearching}
        />
        <button 
          type="submit" 
          disabled={isSearching || !query.trim()}
          className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-accent-purple hover:bg-purple-600 disabled:bg-surface text-textPrimary px-6 py-3 rounded-xl font-bold transition-all shadow-lg flex items-center gap-2"
        >
      ...
      {!isSearching && steps.length === 0 && !result && (
        <div className="flex flex-wrap gap-2 mt-2">
          <span className="text-sm text-textSecondary font-medium w-full mb-1">Try asking:</span>
          {["What are the early symptoms of dengue?", "How can I prevent high blood pressure?", "What should I do for a mild fever?", "Is vitamin B12 deficiency common?"].map((suggestion, idx) => (
            <button
              ...
              className="bg-card border border-border hover:border-[#8B5CF6] hover:bg-accent-purple text-textSecondary px-4 py-2 rounded-xl text-sm transition-all"
            >
              {suggestion}
            </button>
          ))}
      ...
      {steps.length > 0 && (
        <div className="bg-card border border-border rounded-3xl p-6 shadow-xl flex flex-col gap-4">
          <h3 className="font-bold text-textSecondary flex items-center gap-2">
          ...
          <div className="flex flex-col gap-3 pl-2 border-l-2 border-border ml-2">
            {steps.map(step => (
              <div key={step.id} className="flex items-center gap-3">
                {step.status === 'pending' ? (
                  <div className="w-4 h-4 rounded-full border-2 border-border bg-transparent" />
                ) : step.status === 'active' ? (
                ...
                <div>
                  <span className={`text-sm font-bold ${step.status === 'complete' ? 'text-textSecondary' : 'text-textPrimary'}`}>
                    {step.source}
                  </span>
                  <span className={`text-sm ml-2 ${step.status === 'complete' ? 'text-textTertiary' : 'text-textSecondary'}`}>
                    {step.message}
                  </span>
                </div>
              </div>
      ...
      {result && (
        <div className="bg-card border border-[#8B5CF6]/30 rounded-3xl p-8 shadow-[0_0_30px_rgba(139,92,246,0.1)] flex flex-col gap-6 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-[#8B5CF6] to-[#00C9A7]" />
          
          <div className="prose prose-invert max-w-none">
            {result.text.split('\n\n').map((paragraph, i) => (
              <p key={i} className="text-textPrimary leading-relaxed text-lg mb-4" dangerouslySetInnerHTML={{ __html: paragraph.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
            ))}
          </div>

          {result.sources.length > 0 && (
            <div className="mt-4 pt-6 border-t border-border">
              <h4 className="text-sm font-bold text-textSecondary mb-3 uppercase tracking-wider flex items-center gap-2">
                <ShieldCheck size={16} /> Verified Sources
              </h4>
              <div className="flex flex-wrap gap-2">
                {result.sources.map((src, i) => (
                  <div key={i} className="bg-surface border border-border rounded-lg px-4 py-2 flex flex-col">
                    <span className="text-sm font-bold text-textSecondary">{src.name}</span>
