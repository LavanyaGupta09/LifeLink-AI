  ...
  return (
    <div className={`w-full min-h-screen bg-background text-textPrimary font-sans flex flex-col relative pb-[120px] md:pb-0 px-6 py-6 transition-all duration-300 ${isSidebarExpanded ? 'md:pl-72' : ''}`}>
      ...
      <header className="w-full flex justify-between items-center p-6 lg:px-10 lg:py-8 border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div className="w-14 h-14 rounded-full bg-fuchsia-600 flex items-center justify-center font-bold text-2xl shadow-lg shadow-fuchsia-600/20">
            <FlaskConical size={28} className="text-textPrimary" />
          </div>
          <div>
            <h1 className="text-xl md:text-3xl font-bold tracking-tight">Diagnostics Lab</h1>
            <p className="text-textSecondary text-sm md:text-base mt-1">Partner Portal • NABL Accredited</p>
          </div>
        </div>
      ...
      <main className="w-full flex-1 p-6 lg:p-10 flex flex-col min-h-0">
        ...
        {activeTab === 'uploader' && (
          <div className="flex-1 w-full flex flex-col">
            <div className="mb-6">
              <h2 className="text-3xl font-bold text-textPrimary mb-2">Patient Vault Uploader</h2>
              <p className="text-textSecondary text-lg">Securely push encrypted PDF lab reports directly into the patient's personal health vault.</p>
            </div>

            <div 
              className={`flex-1 w-full bg-card border-4 border-dashed rounded-3xl p-10 lg:p-16 flex flex-col items-center justify-center transition-all shadow-xl ${dragActive ? 'border-fuchsia-500 bg-background' : 'border-border'} ${uploadStatus === 'success' ? 'border-emerald-500 bg-[#0f241a]' : ''}`}
            ...
            >
              {uploadStatus === 'success' ? (
                <div className="flex flex-col items-center text-center animate-in zoom-in duration-300">
                  ...
                  <h3 className="text-3xl font-black text-textPrimary mb-3">Upload & Analysis Complete</h3>
                  <p className="text-emerald-400 text-lg mb-8">Report securely encrypted and pushed to patient Vault.</p>
                  
                  <div className="bg-background border border-rose-500/30 rounded-2xl p-6 w-full max-w-3xl text-left mb-8 shadow-xl">
                    ...
                    <p className="text-textSecondary text-lg font-medium leading-relaxed">Critical Finding: Fasting Blood Sugar is <span className="text-rose-400 font-bold">210 mg/dL</span>. Elevated risk for severe hyperglycemia. Patient's primary care physician has been automatically alerted.</p>
                  </div>
                  
                  <button onClick={() => setUploadStatus(null)} className="text-textSecondary font-bold hover:text-textPrimary underline text-lg">Upload another report</button>
                </div>
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-3xl font-black text-textPrimary mb-3">AI Scanning...</h3>
                  <p className="text-textSecondary text-lg">Gemini 2.0 is extracting text and checking for critical red-flag values.</p>
                </div>
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-3xl font-black text-textPrimary mb-3">Zero-Knowledge Encryption</h3>
                  <p className="text-textSecondary text-lg">Securing report with patient's public key before vault injection.</p>
                </div>
              ) : (
                <div className="flex flex-col items-center text-center">
                  <div className="w-32 h-32 bg-background rounded-full flex items-center justify-center mb-8 text-textTertiary border border-border shadow-inner">
                    <UploadCloud size={64} />
                  </div>
                  <h3 className="text-3xl font-black text-textPrimary mb-3">Drag & Drop Lab Report</h3>
                  <p className="text-textSecondary text-lg mb-10 max-w-md">Supports PDF, DICOM, or HL7 messages up to 50MB.</p>
                  
                  {errorMsg && <p className="text-red-500 font-bold mb-6 animate-fade-in text-lg">{errorMsg}</p>}
                  
                  <label className="bg-fuchsia-600 hover:bg-fuchsia-500 text-textPrimary px-10 py-5 rounded-2xl font-black text-lg transition-transform active:scale-95 shadow-[0_0_30px_rgba(192,38,211,0.3)] cursor-pointer">
                    BROWSE FILES
        ...
        {activeTab === 'catalog' && (
          <div className="flex-1 w-full flex flex-col">
            <header className="flex justify-between items-end mb-8">
              <div>
                <h2 className="text-3xl font-bold text-textPrimary mb-2">Test Catalog & Pricing</h2>
                <p className="text-textSecondary text-lg">Manage available diagnostic tests, pricing, and home collection options.</p>
              </div>
              <button 
                onClick={saveCatalog}
                className="bg-fuchsia-600 hover:bg-fuchsia-500 text-textPrimary font-bold py-3 px-8 rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-fuchsia-600/20 active:scale-95"
              >
            ...
            <div className="bg-card rounded-3xl shadow-xl border border-border overflow-hidden flex-1">
              <table className="w-full text-left text-sm lg:text-base">
                <thead>
                  <tr className="bg-background border-b border-border text-xs text-textSecondary uppercase tracking-widest font-black">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {tests.map(test => (
                    <tr key={test.id} className="hover:bg-surface transition-colors">
                      <td className="px-8 py-5 font-bold text-textPrimary">{test.name}</td>
                      <td className="px-8 py-5 text-textTertiary font-medium">{test.category}</td>
                      <td className="px-8 py-5">
                        <div className="relative w-32">
                          ...
                          <input 
                            ...
                            className="w-full bg-background border border-border rounded-xl pl-10 pr-4 py-3 outline-none focus:border-fuchsia-500 disabled:opacity-50 text-textPrimary font-bold transition-colors"
                          />
                        </div>
                      </td>
        ...
        {activeTab === 'fleet' && (
          <div className="flex-1 w-full flex flex-col">
            <header className="mb-8">
              <h2 className="text-3xl font-bold text-textPrimary mb-2">Phlebotomist Fleet Map</h2>
              <p className="text-textSecondary text-lg">Live GPS tracking for home collection agents.</p>
            </header>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="w-full lg:col-span-4 flex flex-col gap-4 overflow-y-auto pr-2">
                {MOCK_FLEET.map(agent => (
                  <div key={agent.id} className="bg-card p-6 rounded-3xl shadow-xl border border-border">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-bold text-textPrimary text-lg">{agent.name}</h4>
                      <span className={`text-xs font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border ${
                        agent.status === 'Offline' ? 'bg-surface text-textSecondary border-border' :
                    ...
                    {agent.status !== 'Offline' && (
                      <div className="text-base text-textSecondary font-medium space-y-2 bg-background p-4 rounded-2xl border border-border">
              ...
              <div className="w-full lg:col-span-8 bg-background rounded-3xl border border-border relative overflow-hidden flex items-center justify-center shadow-xl min-h-[500px]">
                ...
                {MOCK_FLEET.map(agent => (
                  <div key={agent.id} className="absolute" style={{ top: `${agent.lat}%`, left: `${agent.lng}%`, transform: 'translate(-50%, -50%)' }}>
                    <div className="relative group cursor-pointer">
                      <div className={`w-5 h-5 rounded-full border-2 border-border shadow-[0_0_15px_rgba(0,0,0,0.5)] ${agent.status === 'Offline' ? 'bg-slate-500' : agent.status === 'En Route' ? 'bg-sky-500' : agent.status === 'Collecting' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                      {agent.status !== 'Offline' && (
                      ...
                      {/* Tooltip */}
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-40 bg-card border border-border text-textPrimary p-3 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-2xl">
                        <p className="font-bold text-sm mb-1">{agent.name}</p>
                        <p className="text-textSecondary text-xs font-black uppercase tracking-widest">{agent.status}</p>
                      </div>
                    </div>
                  </div>
                ))}
                
                <div className="absolute bottom-6 right-6 bg-card backdrop-blur-md p-5 rounded-2xl shadow-2xl border border-border text-xs font-bold text-textSecondary uppercase tracking-widest">
        ...
        {activeTab === 'sms' && (
          <div className="flex-1 w-full flex flex-col">
            <header className="flex justify-between items-end mb-8">
              <div>
                <h2 className="text-3xl font-bold text-textPrimary mb-2">Auto-SMS Dispatch</h2>
                <p className="text-textSecondary text-lg">Automatically notify patients when their lab reports are uploaded to their Vault.</p>
              </div>
              <div className="flex items-center gap-4 bg-card px-6 py-3 rounded-2xl border border-border shadow-lg">
                <span className="text-sm font-black uppercase tracking-widest text-textSecondary">Master Toggle:</span>
                <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-card rounded-3xl shadow-xl border border-border p-8 flex-1 flex flex-col">
                  <h3 className="font-bold text-textPrimary mb-6 flex items-center gap-3 text-xl"><MessageSquare size={24} className="text-fuchsia-500"/> SMS Template</h3>
                  <div className="bg-background p-6 rounded-2xl border border-border font-mono text-base text-textSecondary leading-loose flex-1 shadow-inner">
                  ...
                  <p className="text-sm text-textTertiary mt-6 flex items-center gap-2 font-medium"><AlertTriangle size={16} className="text-amber-500"/> Variables in brackets are auto-filled per patient.</p>
                </div>
                
                <button className="w-full border-2 border-fuchsia-600 text-fuchsia-500 hover:bg-fuchsia-600 hover:text-textPrimary font-black text-lg py-5 rounded-2xl transition-all shadow-lg active:scale-95">
                  EDIT TEMPLATE
                </button>
              </div>

              {/* Delivery Log */}
              <div className="bg-card rounded-3xl shadow-xl border border-border overflow-y-auto flex flex-col min-h-screen w-full">
                <div className="px-8 py-6 border-b border-border bg-background"><h3 className="font-bold text-textPrimary text-lg">Recent Dispatch Log</h3></div>
                <div className="divide-y divide-slate-800/50 flex-1 overflow-y-auto">
                  {MOCK_SMS.map(log => (
                    <div key={log.id} className="p-6 flex items-center justify-between hover:bg-surface transition-colors">
                      <div>
                        <p className="font-bold text-base text-textPrimary flex items-center gap-3 mb-1"><Phone size={16} className="text-textTertiary"/> {log.phone}</p>
                        <p className="text-sm text-textSecondary font-medium">{log.reportName} <span className="text-textTertiary mx-2">•</span> {log.time}</p>
                      </div>
      ...
      <div className={`fixed bottom-6 md:bottom-auto md:top-0 md:left-0 md:h-full w-[90%] left-[5%] md:left-0 bg-card backdrop-blur-xl border border-border md:border-y-0 md:border-l-0 md:border-r md:rounded-none rounded-full px-4 py-3 md:px-0 md:py-6 flex md:flex-col justify-between md:justify-start items-center gap-0 md:gap-4 shadow-2xl z-50 overflow-x-auto md:overflow-visible transition-transform duration-300 ${isSidebarExpanded ? 'md:w-64 md:items-start md:px-4 md:translate-x-0' : 'md:w-64 md:-translate-x-full'}`}>
        ...
        <button onClick={() => setActiveTab('uploader')} className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'uploader' ? 'text-fuchsia-500 md:bg-fuchsia-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'VAULT' : undefined}>
        ...
        <button onClick={() => setActiveTab('catalog')} className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'catalog' ? 'text-fuchsia-500 md:bg-fuchsia-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'TESTS' : undefined}>
        ...
        <button onClick={() => setActiveTab('fleet')} className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'fleet' ? 'text-fuchsia-500 md:bg-fuchsia-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'FLEET' : undefined}>
        ...
        <button onClick={() => setActiveTab('sms')} className={`flex items-center active:scale-95 transition-all w-full px-2 md:px-3 py-2 md:py-3 md:rounded-xl ${activeTab === 'sms' ? 'text-fuchsia-500 md:bg-fuchsia-500/10' : 'text-textTertiary hover:text-textSecondary md:hover:bg-surface'} ${!isSidebarExpanded ? 'flex-col md:justify-center' : 'md:justify-start md:flex-row md:gap-3'}`} title={!isSidebarExpanded ? 'SMS' : undefined}>
