      ...
      <div className="sticky top-0 z-40 w-full bg-background backdrop-blur-md border-b border-border p-4">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-textSecondary">Manage policies & applications</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-8">
        ...
        <section>
          ...
          {activePolicies.length === 0 ? (
            <div className="bg-card border border-border rounded-2xl p-6 text-center">
              <p className="text-textSecondary">No active policies found.</p>
            </div>
          ) : (
                ...
                return (
                  <div key={policy.id} className="bg-gradient-to-br from-surface to-background border border-emerald-500/30 rounded-3xl p-6 shadow-lg relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10">
                      <div>
                        <p className="text-[10px] font-bold text-textSecondary uppercase tracking-widest mb-1">{planDetails?.provider || 'Insurance Provider'}</p>
                        <h3 className="text-xl font-black text-textPrimary">{planDetails?.planName || 'Health Plan'}</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 relative z-10">
                      <div>
                        <p className="text-[10px] text-textTertiary uppercase mb-1">Policy Number</p>
                        <p className="font-mono text-sm font-bold text-textPrimary">{policy.policyNumber}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-textTertiary uppercase mb-1">Coverage</p>
                        <p className="text-sm font-bold text-emerald-400">₹{planDetails ? (planDetails.coverage / 100000) : 10} Lakh</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-textTertiary uppercase mb-1">Renewal Date</p>
                        <p className="text-sm font-bold text-textPrimary">{policy.renewalDate}</p>
                      </div>
                    </div>

                    <div className="border-t border-border pt-4 mb-6 relative z-10">
                      <p className="text-[10px] text-textTertiary uppercase mb-2">Members Covered</p>
                      <div className="flex flex-wrap gap-2">
                        {policy.membersCovered.map((m: string, i: number) => (
                          <span key={i} className="bg-background border border-border text-textSecondary text-xs px-3 py-1 rounded-full">{m}</span>
                        ))}
                      </div>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-3 relative z-10">
                      <button className="flex-1 bg-surface hover:bg-surface text-textPrimary py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors">
        ...
        <section>
          ...
          {applications.length === 0 ? (
            <div className="bg-card border border-border rounded-2xl p-6 text-center">
              <p className="text-textSecondary">No recent applications.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {applications.map(app => {
                ...
                return (
                  <div key={app.id} className="bg-card border border-border rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-xs text-textTertiary">{app.id}</span>
                        <span className="bg-amber-500/20 text-amber-500 text-[10px] font-bold px-2 py-0.5 rounded border border-amber-500/30">
                          {app.status}
                        </span>
                      </div>
                      <h3 className="font-bold text-textPrimary">{planDetails?.planName || 'Insurance Plan'}</h3>
                      <p className="text-xs text-textSecondary">Applicant: {app.applicantName}</p>
                    </div>
                    
                    <div className="flex flex-col sm:items-end gap-1 w-full sm:w-auto">
                      <p className="text-xs text-textTertiary flex items-center gap-1">
